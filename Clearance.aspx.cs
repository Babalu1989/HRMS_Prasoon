...
public partial class Clearance : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {           
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                DataRow dr = dtddl_vendor.NewRow();
                dr = dtddl_vendor.NewRow();
                dr["VENDOR_NAME"] = "CTC (BRPL) ";
                dr["VENDOR_CODE"] = "CTC";
                dtddl_vendor.Rows.InsertAt(dr, 0);

                DataRow drgpa = dtddl_vendor.NewRow();
                drgpa = dtddl_vendor.NewRow();
                drgpa["VENDOR_NAME"] = "GPA";
                drgpa["VENDOR_CODE"] = "GPA";
                dtddl_vendor.Rows.InsertAt(drgpa, 1);
                dbcon.FillDDLwithdt(ddl_vendor_popup, dtddl_vendor);
            }
    ...
    protected void gv_uploaddata_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            //int y = 0;
            //int gv_uploaddatarowscnt = gv_uploaddata.Rows.Count;

            //foreach (GridViewRow row in gv_uploaddata.Rows)
            //{
            //    for (int z = 0; z < gv_uploaddatarowscnt; z++)
            //    {
            //        var checkItem = gv_uploaddata.Rows[z].Cells[2].Text;
            //        string catid = row.Cells[2].Text;
            //        if (catid == checkItem)    //or use String.compare here
            //        {
            //            y = y + 1;
            //            row.Cells[2].BackColor = System.Drawing.ColorTranslator.FromHtml("#FF0000");
            //            row.Cells[2].ToolTip = "EmpId Duplicate";
            //        }
            //    }
            //} 
    ...
    [WebMethod]
    public static IEnumerable clearancedsave(string typ, string empid, string name, string roles, string company, string dob, string aadhar, string doj, string department, string location, string dos,string dosreason, string mobile, string email, string liempid, string liname, string limobile, string liemail)
    {
        DataSet ds = new DataSet();
        DataTable dt = new DataTable();
        ds = dbconStatic.Exe_Proc_with_PROC_EMP_MASTER_MNG(typ, aadhar, dtlogin.Rows[0]["user_loginid"].ToString(), empid, name, roles, company, dob, aadhar, doj, department, location, dos, dosreason,mobile,email,liempid,liname,limobile,liemail, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "");
        dt = ds.Tables[0];
        if (dt.Rows.Count > 0)
        {
    ...
    {
     string filename = "SeperationDetails.xlsx";
