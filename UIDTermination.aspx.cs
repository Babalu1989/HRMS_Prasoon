...
public partial class UIDTermination : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!Page.IsPostBack)
            { Session["EMP_CODE"] = dtlogin.Rows[0]["USER_LOGINID"].ToString(); Session["EMP_TYPE"] = dtlogin.Rows[0]["MRFUSERTYP"].ToString(); }
        }
        catch (Exception er) {
    ...
    [WebMethod]
    public static IEnumerable employeeterminate(string typ, string aadhar, string status, string UID, string reason)
    {

        DataSet ds = new DataSet();
        DataTable dt = new DataTable();
        ds = dbconStatic.Exe_Proc_with_PROC_HRMS_MRF(typ,aadhar, dtlogin.Rows[0]["user_loginid"].ToString(),status,reason);      
        dt = ds.Tables[0];
        if (dt.Rows.Count > 0)
        {
