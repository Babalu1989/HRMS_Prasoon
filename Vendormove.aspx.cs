...
public partial class Vendormove : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       // oraconstatic = new OracleConnection(ConfigurationManager.ConnectionStrings["connection"].ConnectionString);
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                dbcon.FillDDLwithdt1(ddl_vendor, dtven);
                dbcon.FillDDLwithdt1(ddl_updatevendor, dtven);

                DataTable dtdiv = new DataTable();
                dt.DefaultView.RowFilter = "TXT ='DIV'";
                dtdiv = (dt.DefaultView).ToTable();
                dbcon.FillDDLwithdt1(ddl_div, dtdiv);
                dbcon.FillDDLwithdt1(ddl_update_div, dtdiv);

                DataTable dtddl_noj = new DataTable();
                dt.DefaultView.RowFilter = "TXT ='NATJOB'";
                dtddl_noj = (dt.DefaultView).ToTable();              
                dbcon.FillDDLwithdt1(ddl_noj, dtddl_noj);
            }
        }
    }
    ...
    [WebMethod]
    public static IEnumerable oldemplist(string div, string subdiv, string ven)
    {
        DataTable value = new DataTable();
        string qry2 = "select EMP_NM,AADHAR,UIDNO,DIV_CODE,SUB_DIV from HRMS_FULL_EMP_DETAIL where status = 0 ";
        if (ven != "0")
        {
            qry2 = qry2 + " and vendor_code ='" + ven + "'";

        }
        if (subdiv != "")
        {
            qry2 = qry2 + " and sub_div_code='" + subdiv + "'";

        }
        if (div != "0")
        {
            qry2 = qry2 + " and div_code='" + div + "'";

        }
        qry2 = qry2 + "order by EMP_NM";
        OracleDataAdapter oda = new OracleDataAdapter(qry2, oraconstatic);
        oda.Fill(value);
    ...
    public static IEnumerable updatelist(newlist[] newlist)
    {
        ...
        {
            resultds = dbconStatic.Exe_Proc_with_PROC_EMP_MASTER_MNG(item.typ, item.aadhar.Trim(), dtlogin.Rows[0]["user_loginid"].ToString(), item.vendor.Trim(), item.division, item.subivision, item.effectivedate,"", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "");
    ...
    public class newlist
    {
        ...
        public string subivision { get; set; }
